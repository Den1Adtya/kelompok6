...
<nav class="flex justify-around items-center py-10">
    ...
    <div class="logo flex items-center justify-center gap-5">
        <img src="{{asset('assets/image/logo.png')}}" width="30" alt="logoPinterest" />
        <h1 class="font-bold text-[15px]">WEBSITE IMAGE</h1>
    </div></a>

    <ul class=" justify-center items-center gap-10  lg:flex">
        <li>
            <a href="/dashboard" class="text-black font-bold flex justify-center items-center gap-2">
...
<div class="relative min-h-screen flex flex-wrap gap-10  justify-center  py-12 px-4 sm:px-6 lg:px-8  bg-no-repeat bg-cover items-start">
    {{-- card upload image --}}
	<div class="sm:max-w-lg w-full p-10 shadow-md border-2  border-[#002D74] bg-white rounded-xl z-10">
...
<div class="flex flex-col gap-5">
    ...
    <div>
      <span class="text-white text-xs font-bold rounded-lg bg-[#002D74] inline-block mt-4 ml-4 py-1.5 px-4 cursor-pointer">kategori image</span>
      @if ($errors->has('lokasiFile'))
      <h1 class="text-2xl mt-2 ml-4 font-bold text-[#002D74] cursor-pointer  transition duration-100">{{ $errors->first('judulFoto') }}</h1>
      @else
    ...
    <div class="z-10 absolute  mb-10 m-2  px-2 bg-[#002D74] text-white rounded-full py-2 text-[10px] ">( PNG, JPG/JPEG , SVG )</div>

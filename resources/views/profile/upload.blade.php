...
<nav class="flex justify-around items-center py-5">
    ...
    <ul class=" justify-center items-center gap-10  lg:flex">
        <li>
            <a href="/dashboard" class="text-[#011f4b] font-bold flex justify-center items-center gap-2">
...
{{-- content --}}
<div class="relative min-h-screen flex flex-wrap gap-10  justify-center  py-3 px-4 sm:px-6 lg:px-8  bg-no-repeat bg-cover items-start">
    {{-- card upload image --}}
	<div class="sm:max-w-lg w-full p-10 shadow-md border-2  border-[#011f4b] bg-white rounded-xl z-10">
		<div class="text-center">
			<h2 class="mt-5 text-3xl font-bold text-gray-900">
				File Upload!
			</h2>
			<p class="mt-2 text-sm text-gray-400">Upload your image's here</p>
		</div>
        <form action="{{ route('upload')}}" method="POST"  enctype="multipart/form-data">
            @csrf
                    <div class="grid grid-cols-1 space-y-2">
                        <label class="text-sm font-bold text-gray-500 tracking-wide">Image Title</label>
                    ...
                    <div class="grid grid-cols-1 space-y-2">
                        ...
                        <div class="grid grid-cols-1 space-y-2">
                            <label class="text-sm font-bold text-gray-500 tracking-wide">Input Image</label>
                            ...
                            <div class="Remember flex py-2 gap-2 text-[13px] ">
                              <input type="checkbox" class="rounded-lg" name="remember" id="remember" required>
                              <div>
                              <a class="hover:underline " href="/">Agree Terms And Condition</a>  
                              </div>
                          </div>
                        </div>
                    <div>
                        <button type="submit"  class="my-5 w-full flex justify-center bg-[#002D74] text-gray-100 p-4  rounded-full tracking-wide
                                    font-semibold  focus:outline-none focus:shadow-outline hover:bg-[#011f4b] shadow-lg cursor-pointer transition ease-in duration-300">
                        Upload
                    </button>
                    </div>
        </form>
...
<div class="flex flex-col gap-5">
    ...
    <div>
      <span class="text-white text-xs font-bold rounded-lg bg-[#002D74] inline-block mt-4 ml-4 py-1.5 px-4 cursor-pointer">Category Image</span>
    ...
    @else
    <h1 class="text-2xl mt-2 ml-4 font-bold text-gray-800 cursor-pointer hover:text-gray-900 transition duration-100">Image Title ( max 50 )</h1>
    @endif
    @if ($errors->has('deskripsiFoto'))
    <p class="ml-4 mt-1 mb-2 text-gray-700 hover:underline cursor-pointer">{{ $errors->first('deskripsiFoto') }}</p>
    @endif
      <p class="ml-4 mt-1 mb-2 text-gray-700 hover:underline cursor-pointer">Image Description ( max 255 )</p>
    </div>
    <div class="bg-gray-500 p-[100px] ">
    ...
    @endif
    <div class="">
        <h1 class="w-80 mt-4 text-yellow-500">*Attention</h1>
        <p class="text-yellow-300 font-medium">Dilarang Untuk Mengupload Photo Dengan Unsur Kekerasan Dan Berbau Pornografi</p>
    </div>
    </div>
